...
public class Main {
        ...
        for(int i=0; i<resultsBoolean.size() && flag; i++){
            ...
            for(int j=0; j<results.size() && flag; j++){
                ...
                if(result==false){
                    ...
                    printResult(userResults.get(i).get(j));
                    System.out.println("Expected : ");
                    printResult(expected.get(i).get(j));
                    flag = false;
                }
            }
        }
    ...
    public static ArrayList<Boolean> takeInput(int t,ArrayList<Boolean> res){
        ...
        while(count<=n){
            ...
            int[] res1 = sol1.plusOne(nums1); expected_result.add(res1);
            int[] res2 = sol2.plusOne(nums2); user_result.add(res2);
